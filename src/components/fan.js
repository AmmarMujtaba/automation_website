export default function Fan (props){
    ...
    async function updateBtnStatus(){
        console.log('inside updateBtnStatus')
    ...
    setInterval(updateBtnStatus,3000)
    return (
        <>
            <p id='isFanOn'>The fan is <b>{props.status.isFanOn?"ON":"OFF"}</b></p>
            <p id='isBtnOn'>The button is <b>{props.status.isBtnOn?"ON":"OFF"}</b></p>
